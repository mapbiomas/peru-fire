...
class MosaicAssemblerUI:
    ...
    def _on_miss(self, _):
        ...
        with self.out:
            ...
            for y, m, p, _ in pend:
                assemble_country_mosaic(y, m, p)
                self._show_download_links(y, m, p)
    ...
    def _show_download_links(self, year, month, period='monthly'):
        if period == 'monthly':
            prefix = monthly_chunk_path(year, month)
            label = f"{year}-{month:02d}"
        else:
            prefix = f"yearly/chunks/{year}"
            label = f"{year} (Anual)"
        links_html = f"<b>📥 Enlaces GCS ({label}):</b><br>"
