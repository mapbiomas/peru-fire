...
var SECTION_STYLE = {
    ...
    unpromote:  { margin:'4px', padding:'8px', backgroundColor:'#ffeaea', border:'1px solid #f0b8b8', borderRadius:'6px' },
};
...
var STYLE = {
    ...
    contentsCard: { margin:'0 0 6px 0', padding:'6px', backgroundColor:'#fff', border:'1px solid #f0b8b8', borderRadius:'4px' },
...
var promotedChecks = {};         // key: pais__campanha__fecha -> bool
...
var countryBox, campaignBox, dateBox, candidatesBox, protocolBox, promoteBox, unpromoteBox, statusLabel, exportStatus, promoteButton;
...
// ─── UNPROMOTE SECTION (isolated panel, last) ───────────────────────────────

function renderPromotedLine(){
    if(!unpromoteBox)return;
    unpromoteBox.clear();
    unpromoteBox.add(ui.Label(L.unpromote_section, STYLE.sectionTitle));

    var card = ui.Panel({layout:ui.Panel.Layout.flow('vertical'), style:STYLE.contentsCard});
    card.add(ui.Label(L.existing_pre, {fontSize:'10px',fontWeight:'bold',color:'#d32f2f',margin:'0 0 4px 0'}));
    var pairs = activePairs();
    var any = false;
    pairs.forEach(function(pair){
        var c = pair.country, camp = pair.camp;
        ...
        periods.forEach(function(p){
            any = true;
            var row = ui.Panel({layout:ui.Panel.Layout.flow('horizontal'), style:{margin:'1px 0'}});
            var pk = promotedKey(c, camp, p);
            var cb = ui.Checkbox({label:c+' | '+camp+' | '+p+' ✅', value:promotedChecks[pk]||false, style:STYLE.gridCell});
            cb.onChange(function(v){
                promotedChecks[pk] = v;
                if(v){
                    var img = ee.Image(prePublicPath(c, camp)+p).select('probability').selfMask();
                    loadMosaic(c, p);
                    manageMapLayer('promoted_'+pk, img, {min:0,max:1000,palette:['#fcc','#f00','#600']}, 'Promovido '+p+' | '+camp+' | '+c);
                } else {
    ...
    unpromoteBox.add(card);

    var br = ui.Panel({layout:ui.Panel.Layout.flow('horizontal'), style:{margin:'4px 0'}});
    br.add(ui.Button({label:L.unpromote, style:STYLE.btnRed, onClick:function(){showUnpromoteConfirm();}}));
    unpromoteBox.add(br);
}
...
function showUnpromoteConfirm(){
    ...
    checked.forEach(function(k){
        ...
        card.add(ui.Label(parts[0]+' | '+parts[1]+' | '+parts[2], {fontSize:'11px',fontFamily:'monospace',color:'#1a73e8',margin:'1px'}));
    });
...
function buildUnpromoteSection(){
    var section = ui.Panel({layout:ui.Panel.Layout.flow('vertical'), style:SECTION_STYLE.unpromote});
    unpromoteBox = ui.Panel({layout:ui.Panel.Layout.flow('vertical')});
    section.add(unpromoteBox);
    renderPromotedLine();
    return section;
}
...
function buildForm(){
    ...
    sidePanel.add(buildUnpromoteSection());
