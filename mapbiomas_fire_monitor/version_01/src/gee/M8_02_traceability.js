...
function calcBurnedArea(assetId) {
    ...
    var stats = img.multiply(areaImage).reduceRegion({reducer: ee.Reducer.sum(), geometry: REGIONS.geometry(), scale: SCALE, maxPixels: 1e13});
...
print('Period: ' + PERIOD);
print('');
var csv = ['etapa_antes,etapa_depois,area_antes_ha,area_depois_ha,delta_ha,delta_pct'];
...
for (var i = 0; i < STAGES.length - 1; i++) {
    ...
    var areaBefore = 0, areaAfter = 0;
    try { ee.data.getAsset(assetBefore); areaBefore = calcBurnedArea(assetBefore); } catch (e) { print('  [SKIP] Not found: ' + STAGES[i]); continue; }
    try { ee.data.getAsset(assetAfter); areaAfter = calcBurnedArea(assetAfter); } catch (e) { print('  [SKIP] Not found: ' + STAGES[i + 1]); continue; }
